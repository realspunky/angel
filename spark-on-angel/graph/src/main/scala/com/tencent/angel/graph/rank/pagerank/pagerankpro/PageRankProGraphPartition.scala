...
class PageRankProGraphPartition(index: Int,
  ...
  def getDegree() = {
    val deg = new Array[Int](keys.length)
    var i = 1
    while (i < indptr.length) {
      deg(i-1) = indptr(i) - indptr(i - 1)
      i += 1
    }
    keys.zip(deg)
  }

  def start(model: PageRankProPSModel, rank: Float, resetProb: Float, tol: Float): Int = {
    val outMsgs = new Long2FloatOpenHashMap()
    val ranks = model.readRanks(keys.clone())
    for (idx <- keys.indices) {
      val delta = ranks.get(keys(idx))
      if (delta >= tol) {
        var j = indptr(idx)
  ...
  def process(model: PageRankProPSModel, resetProb: Float, tol: Float, numMsgs: Long): Int = {
  ...
  def setMissRanks(model: PageRankProPSModel, initRanks: Float): Int = {
    if (keys.length > 0) {
      val ranks = model.readRanks(keys.clone())
...
object PageRankProGraphPartition {
  def apply(index: Int, iter: Iterator[(Long, Iterable[(Long, Float)])]): PageRankProGraphPartition = {
    ...
    while (iter.hasNext) {
      ...
      outs.toArray.distinct.foreach { case (n, weight) =>
        outNodes.add(n)
        weights.add(weight)
      }
    ...
    new PageRankProGraphPartition(index,
      keys.toLongArray(),
      indptr.toIntArray(),
      sums.toFloatArray(),
