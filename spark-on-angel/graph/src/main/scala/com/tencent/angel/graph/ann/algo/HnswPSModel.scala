...
class HnswPSModel(val queryVecMatrix: PSMatrix,
  ...
  def getQueries(): Long2ObjectOpenHashMap[Array[Float]] = {
    null
  }
...
object HnswPSModel {
  ...
  def apply(modelContext: ModelContext, edges: RDD[(Long, Array[Float])], useBalancePartition: Boolean = false,
            balancePartitionPercent: Float = 0.7f): HnswPSModel = {
    ...
    new HnswPSModel(queryMatrix, resMatrix)
