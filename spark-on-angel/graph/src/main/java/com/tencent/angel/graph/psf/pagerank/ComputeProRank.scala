...
class ComputeProRank(param: MMUpdateParam) extends MMUpdateFunc(param) {
  ...
  def update(rows: Array[ServerRow],  scalars: Array[Double]): Unit = {
    ...
    (outMsgs, ranks) match {
      case (o: IntFloatVector, r: IntFloatVector) =>
        compute(o, r, scalars(1).toFloat, scalars(2).toFloat)
  ...
  def compute(writeMsgs: IntFloatVector, ranks: IntFloatVector, resetProb: Float, resizeFlag: Float): Unit = {
    ...
    while (it.hasNext) {
      ...
      val old = ranks.get(key)
      ranks.set(key, old + msg * (1 - resetProb))
    }
  }

  def compute(writeMsgs: LongFloatVector, ranks: LongFloatVector, resetProb: Float, resizeFlag: Float): Unit = {
    if (resizeFlag > 0 && ranks.size() == 0)
